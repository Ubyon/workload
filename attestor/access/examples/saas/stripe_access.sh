...
printf "## Deploy the workload -->  kubectl apply -f stripe-job.yaml\n"
kubectl apply -f /home/ubuntu/tln/internal2cdemo/podspecs/srcapp/stripe-job.yaml

# Wait for the job to complete
printf "_______________________________________________________________\n"
printf "## Waiting for the job to fetch the credentials with the help of Ubyon's attestor...\n"
#kubectl wait --for=condition=complete --timeout=60s job/apikeyshello -n internal2cdemo
wo=$(kubectl wait --for=condition=complete --timeout=30s job/apikeyshello -n internal2cdemo 2>&1)
printf "## %s...\n" "$wo"
if [[ $wo == *"error"* ]]; then
...
kubectl logs $pod_name -c apikeys-wlinitcontainer -n internal2cdemo --tail 100 -f > credentials.log &
...
# Step 6: Get the logs from the apikeyshello pod
printf "_______________________________________________________________\n"
printf "## Output from the workload logs...\n"
kubectl logs $pod_name -c apikeyshello -n internal2cdemo --tail 100 -f

# Step 7: Clean up
rm credentials.log
kubectl logs $pod_name -c apikeys-wlinitcontainer -n internal2cdemo > /dev/null 2>&1