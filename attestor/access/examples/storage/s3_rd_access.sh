...
printf "## Deploy the workload -->  kubectl apply -f s3hello-job.yaml\n"
kubectl apply -f /home/ubuntu/tln/internal2cdemo/podspecs/srcapp/s3hello-job.yaml

# Wait for the job to complete
printf "____________________________________________________________________________________\n"
printf "## Waiting for the job to fetch the credentials...\n"
#kubectl wait --for=condition=complete --timeout=60s job/s3hello -n internal2cdemo
wo=$(kubectl wait --for=condition=complete --timeout=35s job/s3hello -n internal2cdemo 2>&1)
printf "## %s...\n" "$wo"
if [[ $wo == *"error"* ]]; then
...
pod_name=$(kubectl get pods -n internal2cdemo -o=jsonpath='{.items[0].metadata.name}' -l job-name=s3hello)
...
# Step 6: Get the logs from the s3hello pod
printf "____________________________________________________________________________________\n"
printf "## Output from the workload logs...\n"
kubectl logs $pod_name -c s3hello -n internal2cdemo --tail 100 -f
